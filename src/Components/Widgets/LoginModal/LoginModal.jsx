...
export default function LoginModal() {
  ...
  return (
    <div div id='modal' style={{ display: 'none' }} onClick={(e) => { if (e.target.id === 'background') { document.getElementById('modal').style.display = 'none' } }}>
      <div
      ...
      >
        {forgot ? (
          <ForgotPassword forgot={forgot} setForgot={setForgot} />
        ) : (
          <form action={(e) => {
            e.preventDefault();
            onsubmit(e);
            ...
            {!forgot ? (
              <div className=' w-11/12'>
                ...
                <h1 onClick={() => setForgot(true)} className=' text-sm text-blue-600 cursor-pointer hover:text-blue-700'>Quên mật khẩu?</h1>
              </div>
            ) : (
              <></>
            )}
